/*************************************************** 
  ...
  Ruthlessly stolen and adapted to my use by Leo Nicolle
 ****************************************************/
...
boolean ClickWheel::begin(uint8_t i2caddr) {
  Wire.begin();
...
void ClickWheel::setThreshholds(uint8_t touch, uint8_t release) {
...
uint16_t  ClickWheel::filteredData(uint8_t t) {
  if (t > 12) return 0;
  return readRegister16(MPR121_FILTDATA_0L + t*2);
}

uint16_t  ClickWheel::baselineData(uint8_t t) {
  if (t > 12) return 0;
  uint16_t bl = readRegister8(MPR121_BASELINE_0 + t);
  return (bl << 2);
}

uint16_t  ClickWheel::touched(void) {
...
/*********************************************************************/
// Touch Wheel
void ClickWheel::initTouchWheel(uint8_t numWheelPads, int16_t degreeOffset, uint8_t fPadIndex = 0) {
  firstPadIndex = fPadIndex;
  wheelPads = numWheelPads;
  wheelDegreeOffset = degreeOffset;
  lastWheelAngle = -1;
}

void ClickWheel::takeWheelBaseline(void) {
  for (uint8_t i=0; i<wheelPads; i++) {
    wheelBaseline[i] = filteredData(i + firstPadIndex);
  } 
  Serial.print ("baselines: \t");
  for (uint8_t i=0; i<wheelPads; i++) {
    Serial.print(filteredData(i + firstPadIndex), DEC);
    Serial.print("\t");
  } 
  Serial.println ("");
}

int16_t ClickWheel::getWheelAngle(void) {
  // Retrieve difference scores
  int16_t wheelDeltas[8];
  for (uint8_t i=0; i<wheelPads; i++) {
    wheelDeltas[i] = wheelBaseline[i] - filteredData(i + firstPadIndex);
  } 
...
/*********************************************************************/
boolean ClickWheel::isButtonPressed(uint8_t channel) {
...
uint8_t ClickWheel::readRegister8(uint8_t reg) {
...
uint16_t ClickWheel::readRegister16(uint8_t reg) {
    Wire.beginTransmission(_i2caddr);
    Wire.write(reg);
    Wire.endTransmission(false);
...
*/
/**************************************************************************/
void ClickWheel::writeRegister(uint8_t reg, uint8_t value) {
    Wire.beginTransmission(_i2caddr);
